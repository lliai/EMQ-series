...
from bit_config import *  # noqa: F403,F401
from pytorchcv.model_provider import get_model as ptcv_get_model
from utils import *  # noqa: F403,F401
from utils.models import (q_inceptionv3, q_mobilenetv2_w1, q_resnet18,
                          q_resnet50, q_resnet101)
...
parser.add_argument(
    '--bias_bit', type=int, default=32, help='quantizaiton bit-width for bias')
parser.add_argument(
    '--distill_method',
    type=str,
    default='None',
...
quantize_arch_dict = {
    'resnet50': q_resnet50,
    'resnet50b': q_resnet50,
    'resnet18': q_resnet18,
    'resnet101': q_resnet101,
...
logging.basicConfig(
    ...
    datefmt='%d-%b-%y %H:%M:%S',
    filename=args.save_path + 'log.log')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained and not args.resume:
        logging.info("=> using pre-trained PyTorchCV model '{}'".format(
            args.arch))
        model = ptcv_get_model(args.arch, pretrained=True)
        if args.distill_method != 'None':
            logging.info("=> using pre-trained PyTorchCV teacher '{}'".format(
                args.teacher_arch))
        ...
        if args.distill_method != 'None':
            logging.info("=> creating PyTorchCV teacher '{}'".format(
                args.teacher_arch))
            teacher = ptcv_get_model(args.teacher_arch, pretrained=False)

    if args.quant_scheme == 'modelsize_6.7_a6_75B':
        path = '/home/stack/.torch/models/resnet18-0896-77a56f15.pth'
    ...
    bit_config = bit_config_dict['bit_config_' + args.arch + '_' +
                                 args.quant_scheme]
    name_counter = 0
    ...
    if args.resume and args.resume_quantize:
        if os.path.isfile(args.resume):
            logging.info("=> loading quantized checkpoint '{}'".format(
                args.resume))
            checkpoint = torch.load(args.resume)['state_dict']
            modified_dict = {}
            for key, value in checkpoint.items():
            ...
            logging.info("=> no quantized checkpoint found at '{}'".format(
                args.resume))
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            logging.info(
                "=> loaded optimizer and meta information from checkpoint '{}' (epoch {})"
    ...
    if args.data_percentage == 1:
        ...
        partial_train_dataset, _ = torch.utils.data.random_split(
            train_dataset,
            [dataset_length,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.distill_method != 'None':
            train_kd(train_loader, model, teacher, criterion, optimizer, epoch,
                     val_loader, args, ngpus_per_node, dataset_length)
        else:
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        new_lr = adjust_learning_rate(optimizer, epoch, args, i,
                                      len(train_loader))
...
def train_kd(train_loader, model, teacher, criterion, optimizer, epoch,
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0 and args.rank == 0:
            print(
                'Epoch {epoch_} [{iters}]  Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}'
                .format(epoch_=epoch, iters=i, top1=top1, top5=top5))

        if i % ((dataset_length // (args.batch_size * args.evaluate_times)) +
                2) == 0 and i > 0 and args.evaluate_times > 0:
            acc1 = validate(val_loader, model, criterion, args)
    ...
    torch.save(
        {
            ...
            'fc_scaling_factor': {
                k: v
                for k, v in model.state_dict().items()
                if 'fc_scaling_factor' in k
            },
            'weight_integer': {
                k: v
                for k, v in model.state_dict().items() if 'weight_integer' in k
            },
            'bias_integer': {
                k: v
                for k, v in model.state_dict().items() if 'bias_integer' in k
            },
            'act_scaling_factor': {
                k: v
                for k, v in model.state_dict().items()
                if 'act_scaling_factor' in k
            },
