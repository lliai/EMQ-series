...
from .quant_utils import *  # noqa: F403,F401
    ...
    def forward(self, x, prev_act_scaling_factor=None):
        ...
        if not self.full_precision_flag:
            if self.quant_mode == 'symmetric':
                ...
                bias_scaling_factor = self.fc_scaling_factor.view(
                    1, -1) * prev_act_scaling_factor.view(1, -1)
                self.bias_integer = self.weight_function(
                    self.bias, self.bias_bit, bias_scaling_factor)
            else:
                raise Exception(
                    'For weight, we only support symmetric quantization.')
        else:
            w = self.weight
            b = self.bias
        ...
        return ste_round.apply(
            F.linear(
                x_int, weight=self.weight_integer,
                bias=self.bias_integer)) * correct_output_scale
        ...
        if not self.full_precision_flag:
            if self.quant_mode == 'symmetric':
                self.act_scaling_factor = symmetric_linear_quantization_params(
                    self.activation_bit, self.x_min, self.x_max, False)
            ...
            if (pre_act_scaling_factor is
                    None) or (self.fixed_point_quantization):
                ...
                if identity is None:
                    ...
                    quant_act_int = fixedpoint_fn.apply(
                        x, self.activation_bit, self.quant_mode,
                    ...
                    quant_act_int = fixedpoint_fn.apply(
                        ...
                        pre_weight_scaling_factor, identity,
...
class QuantBnConv2d(Module):
    ...
    def set_param(self, conv, bn):
        ...
        self.register_buffer('convbn_scaling_factor',
                             torch.zeros(self.out_channels))
        self.register_buffer('weight_integer',
                             torch.zeros_like(conv.weight.data))
    ...
    def __repr__(self):
        ...
        s = '({0}, weight_bit={1}, bias_bit={2}, groups={3}, wt-channel-wise={4}, wt-percentile={5}, quant_mode={6})'.format(
            conv_s, self.weight_bit, self.bias_bit, self.conv.groups,
            self.per_channel, self.weight_percentile, self.quant_mode)
        return s
    ...
    def forward(self, x, pre_act_scaling_factor=None):
        ...
        if self.fix_flag is False:
            self.counter += 1
            if (self.fix_BN_threshold is None) or (self.counter <
                                                   self.fix_BN_threshold):
                self.fix_BN = self.training_BN_mode
            else:
        ...
        if not self.fix_BN:
            w_transform = self.conv.weight.data.contiguous().view(
            ...
            conv_scaling_factor = symmetric_linear_quantization_params(
                self.weight_bit, w_min, w_max, self.per_channel)
            weight_integer = self.weight_function(self.conv.weight,
                                                  self.weight_bit,
                                                  conv_scaling_factor)
            conv_output = F.conv2d(
                x, weight_integer, self.conv.bias, self.conv.stride,
                self.conv.padding, self.conv.dilation,
                self.conv.groups) * conv_scaling_factor.view(1, -1, 1, 1)
            ...
            self.bn.running_mean = self.bn.running_mean.detach(
            ) * self.bn.momentum + (1 - self.bn.momentum) * batch_mean
            self.bn.running_var = self.bn.running_var.detach(
            ) * self.bn.momentum + (1 - self.bn.momentum) * batch_var

            output_factor = self.bn.weight.view(
                1, -1, 1, 1) / torch.sqrt(batch_var + self.bn.eps).view(
                    1, -1, 1, 1)
            output = output_factor * (conv_output - batch_mean.view(
                1, -1, 1, 1)) + self.bn.bias.view(1, -1, 1, 1)

            return (output,
                    conv_scaling_factor.view(-1) * output_factor.view(-1))
        # fold BN and fix running statistics
        else:
            running_std = torch.sqrt(self.bn.running_var.detach() +
                                     self.bn.eps)
            scale_factor = self.bn.weight / running_std
            scaled_weight = self.conv.weight * scale_factor.reshape(
            ...
            scaled_bias = (scaled_bias - self.bn.running_mean.detach()
                           ) * scale_factor + self.bn.bias

            if not self.full_precision_flag:
                if self.per_channel:
                    w_transform = scaled_weight.data.contiguous().view(
                        self.conv.out_channels, -1)
                    ...
                    if self.weight_percentile == 0:
                        ...
                        lower_index = math.ceil(input_length *
                                                lower_percentile * 0.01)
                        upper_index = math.ceil(input_length *
                                                upper_percentile * 0.01)

                        w_min = torch.kthvalue(
                            w_transform, k=lower_index, dim=1).values
                        w_max = torch.kthvalue(
                            w_transform, k=upper_index, dim=1).values
                else:
                    if self.weight_percentile == 0:
                        ...
                        w_min, w_max = get_percentile_min_max(
                            scaled_weight.view(-1),
                            100 - self.weight_percentile,
                            self.weight_percentile,
                            output_tensor=True)

                if self.quant_mode == 'symmetric':
                    self.convbn_scaling_factor = symmetric_linear_quantization_params(
                        self.weight_bit, w_min, w_max, self.per_channel)
                    self.weight_integer = self.weight_function(
                        scaled_weight, self.weight_bit,
                        self.convbn_scaling_factor)
                    if self.quantize_bias:
                        bias_scaling_factor = self.convbn_scaling_factor.view(
                            1, -1) * pre_act_scaling_factor.view(1, -1)
                        self.bias_integer = self.weight_function(
                            scaled_bias, self.bias_bit, bias_scaling_factor)
            ...
            return (F.conv2d(x_int, self.weight_integer, self.bias_integer,
                    ...
                    correct_output_scale, self.convbn_scaling_factor)
...
class QuantConv2d(Module):
    ...
    def __repr__(self):
        ...
        s = '(' + s + ' weight_bit={}, full_precision_flag={}, quant_mode={})'.format(
            self.weight_bit, self.full_precision_flag, self.quant_mode)
        return s
    ...
    def set_param(self, conv):
        ...
        self.conv = conv
        self.register_buffer('conv_scaling_factor',
                             torch.zeros(self.out_channels))
    ...
    def forward(self, x, pre_act_scaling_factor=None):
        ...
        if self.quant_mode == 'symmetric':
            self.conv_scaling_factor = symmetric_linear_quantization_params(
                self.weight_bit, w_min, w_max, self.per_channel)
            self.weight_integer = self.weight_function(
                self.weight, self.weight_bit, self.conv_scaling_factor)
        ...
        if self.bias is None:
            return (F.conv2d(x_int, self.weight_integer,
                             torch.zeros_like(bias_scaling_factor.view(-1)),
                             self.conv.stride, self.conv.padding,
                             self.conv.dilation, self.conv.groups) *
