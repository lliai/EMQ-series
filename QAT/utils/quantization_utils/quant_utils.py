...
def get_percentile_min_max(input,
                           lower_percentile,
                           upper_percentile,
                           output_tensor=False):
    """
...
def symmetric_linear_quantization_params(num_bits,
    ...
    with torch.no_grad():
        ...
        if per_channel:
            scale, _ = torch.max(
                torch.stack([saturation_min.abs(),
                             saturation_max.abs()],
                            dim=1),
                dim=1)
...
def asymmetric_linear_quantization_params(num_bits,
    ...
    with torch.no_grad():
        n = 2**num_bits - 1
        scale = torch.clamp(
            (saturation_max - saturation_min), min=1e-8) / float(n)
...
class SymmetricQuantFunction(Function):
    ...
    def forward(ctx, x, k, specified_scale=None):
        ...
        if specified_scale is not None:
            ...
            raise ValueError(
                'The SymmetricQuantFunction requires a pre-calculated scaling factor'
            )
...
class AsymmetricQuantFunction(Function):
    ...
    def forward(ctx, x, k, specified_scale=None, specified_zero_point=None):
        ...
        if specified_scale is not None:
            ...
            raise ValueError(
                'The AsymmetricQuantFunction requires a pre-calculated scaling factor'
            )
...
class fixedpoint_fn(Function):
    ...
    def forward(ctx,
        ...
        with torch.no_grad():
            ...
            if len(z.shape) == 4:
                ...
                pre_act_scaling_factor = transfer_conv_size(
                    pre_act_scaling_factor)
                pre_weight_scaling_factor = transfer_conv_size(
                    pre_weight_scaling_factor)
            elif len(z.shape) == 2:
                z_scaling_factor = transfer_fc_size(z_scaling_factor)
                pre_act_scaling_factor = transfer_fc_size(
                    pre_act_scaling_factor)
                pre_weight_scaling_factor = transfer_fc_size(
                    pre_weight_scaling_factor)
            ctx.z_scaling_factor = z_scaling_factor
            if case == 1:
                if len(z.shape) == 4:
                    identity_scaling_factor = transfer_conv_size(
                        identity_scaling_factor)
                    identity_weight_scaling_factor = transfer_conv_size(
                        identity_weight_scaling_factor)
                elif len(z.shape) == 2:
                    identity_scaling_factor = transfer_fc_size(
                        identity_scaling_factor)
                    identity_weight_scaling_factor = transfer_fc_size(
                        identity_weight_scaling_factor)
            ...
            if case == 0:
                ...
                _A = (
                    pre_act_scaling_factor.type(torch.double) *
                    pre_weight_scaling_factor.type(torch.double))
            ...
            # in case 1, the tensor z should be separeted into 2 parts, one from wy, another from wx
                ...
                _A = (
                    identity_scaling_factor.type(torch.double) *
                    identity_weight_scaling_factor.type(torch.double))
                _B = (_A.type(torch.float)).type(torch.double)
                _C = (z_scaling_factor.type(torch.float)).type(torch.double)
                new_scale = _B / _C
                ...
                wy = (z - identity)
                wy_int = torch.round(wy / pre_act_scaling_factor /
                                     pre_weight_scaling_factor)
    ...
    def backward(ctx, grad_output):
        return grad_output.clone(
        ) / ctx.z_scaling_factor, None, None, None, None, None, None, None, None, None
