...
from quant import (BaseQuantBlock, QuantModel, QuantModule,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        help='dataset name',
        choices=['resnet18', 'mobilenetv2', 'resnet50'])
    parser.add_argument(
        '--batch_size',
        default=64,
        type=int,
    ...
    parser.add_argument(
        '--bit_cfg',
        type=str,
        default='[4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4]')
    ...
    print('prepare training data to perform calibration')
    ...
    if args.test_before_calibration:
        print('get quantized accuracy before calibration')
        # qnn.set_bias_state(args.use_bias, args.vcorr, args.bcorr)
        print('Quantized accuracy before brecq: {}'.format(
    ...
    # Start calibration
    print('start calibration by block reconstruction')
    recon_model(qnn)
    qnn.set_quant_state(weight_quant=True, act_quant=False)
    qnn.set_bias_state(args.use_bias, args.vcorr, args.bcorr)
    print('Weight quantization accuracy: {}'.format(
        validate_model(test_loader, qnn)))
    qnn.set_bias_state(False, False, False)
