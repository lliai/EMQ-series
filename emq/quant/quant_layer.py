...
class QuantModule(nn.Module):
    ...
    def __init__(self,
        ...
        if hasattr(org_module, 'stride'):
            self.stride = org_module.stride
        if hasattr(org_module, 'padding'):
            self.padding = org_module.padding
        if hasattr(org_module, 'dilation'):
            self.dilation = org_module.dilation
        if hasattr(org_module, 'groups'):
        ...
        # replace all activation function to skip connection.
        ...
        # wrapper function
        if getattr(org_module, 'get_block_num', None):
            self.block_num = org_module.get_block_num()
        else:
            self.block_num = 0

        if getattr(org_module, 'get_num_layers', None):
            self.layer_num = org_module.get_num_layers()
        else:
            self.layer_num = 1
    ...
    def forward(self, input: torch.Tensor):
        ...
        # before elemental-wise operation, in that case, we apply
        # activation function and quantization after ele-wise op.
