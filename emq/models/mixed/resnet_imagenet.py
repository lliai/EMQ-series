...
from .base_models import MyNetwork  # noqa: F403,F401
...
class ResNet_ImageNet(MyNetwork):
    ...
    def cfg2flops_layerwise(self, cfg, length, quant_type):
        """bflops calculation.

        Args:
            cfg (_type_): _description_
            length (_type_): _description_
            quant_type (_type_): _description_
        """
        # to simplify, only count convolution flops
        blocks = self.blocks
    ...
    def madnas_forward_pre_GAP(self):
        ...
        for _layer in [self.layer1, self.layer2, self.layer3, self.layer4]:
            for block in _layer:
                output_std_list_plain = block.get_log_zen_score()
