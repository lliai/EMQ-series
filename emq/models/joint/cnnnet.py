...
from .modules import __all_blocks__
...
class CnnNet(Model):
    ...
    def __init__(self,
        ...
        self.fc_linear = nn.Linear(self.stage_channels[-1], num_classes)
    ...
    def get_struct_list(self, struct_info: dict):
        # based on struct info -> get struct list
        struct_list = []
        for block, struct_dict in zip(self.block_list, struct_info):
            num_blocks = block.get_layers()
            if isinstance(struct_dict['nbitsW'], int):
                struct_list.extend([struct_dict['nbitsW']] * num_blocks)
            elif isinstance(struct_dict['nbitsW'], list):
                struct_list.extend(struct_dict['nbitsW'])
        return struct_list
    ...
    def get_stage_info(self, resolution=224):
        ...
        for idx, the_block in enumerate(self.block_list):
            ...
            feature_map_size = the_block.get_output_resolution(
                feature_map_size)
    ...
    def get_efficient_score(self, **kwarg):
        ...
        effective_score_arr = np.exp(
            np.log(depth_arr) - log_width_arr / depth_arr)
