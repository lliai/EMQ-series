...
class ResK1DWK1(nn.Module):
    ...
    def __init__(self,
        ...
        self.model_size += self.conv1.get_model_size() \
            + self.conv2.get_model_size() + self.conv3.get_model_size()
        self.flops += self.conv1.get_flops(1.0) + self.conv2.get_flops(1.0) + self.conv3.get_flops(1.0 / self.stride) \
            + self.bottleneck_channels + self.bottleneck_channels / self.stride ** 2 \
        ...
        if self.in_channels == self.out_channels:
            ...
            self.flops += self.residual_proj.get_flops(
    ...
    def get_flops(self, resolution):
        ...
        # warning: we do not add SE FLOPs=self.se2.get_flops(resolution=None)
        # because will cause bug when setting res=1.0 and calling this function
    ...
    def get_max_feature_num(self, resolution, nbitsA_out=8):
        ...
        conv1_max_featmap = self.conv1.get_max_feature_num(
            resolution, nbitsA_out=self.nbitsA[1])
        conv2_max_featmap = self.conv2.get_max_feature_num(
            resolution, nbitsA_out=self.nbitsA[2]) + residual_featmap
        conv3_max_featmap = self.conv3.get_max_feature_num(
            resolution // self.stride, nbitsA_out=nbitsA_out)
        max_featmap_list = [
    ...
    def get_width(self, ):
        # when dw: c*1^2 * c*k^2/g * c*1^2 = c*k^2*c
        return [
