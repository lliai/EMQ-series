...
class ConvKXBNRELU(ConvKXBN):
    ...
    def get_deepmad_forward(self, **kwarg):
        ...
        return [
            np.log(
                np.sqrt((self.in_channels**kwarg['alpha1']) *
...
class SE(nn.Module):
    ...
    def __init__(self, structure_info, no_create=False, **kwargs):
        '''
        :param structure_info: {
            'class': 'SE',
            'in': in_channels,
        ...
        self.btn_channels = max(
            1,
            round(self.in_channels * self.se_ratio) // 8) * 8
        ...
        self.model_size = self.model_size + self.conv1.get_model_size(
        ) + self.conv2.get_model_size()
