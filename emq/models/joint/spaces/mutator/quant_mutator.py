...
from emq.models.joint.spaces.space_utils import smart_round
from .builder import (MUTATORS, build_btn_mutator, build_channel_mutator,
                      build_kernel_mutator, build_layer_mutator, build_mutator,
                      build_nbits_list_mutator)
...
def revise_nbits_for_layers(old_L, new_L, structure_info):
    ...
    if inner_layers not in [2, 3]:
        raise ValueError(
            'inner_layers must be 2 or 3 for current superblock, not %d' %
            (inner_layers))
    ...
    if old_L < new_L:
        ...
        structure_info['nbitsA'] = structure_info['nbitsA'][:new_L *
                                                            inner_layers]
        structure_info['nbitsW'] = structure_info['nbitsW'][:new_L *
                                                            inner_layers]
...
#     def __init__(self, mutate_method_list, search_channel_list,
#                  search_kernel_size_list, search_layer_list, search_nbits_list,
#                  search_btn_ratio_list, the_maximum_channel, budget_layers,
#                  nbits_ratio, *args, **kwargs):
...
#         kwargs.update(
#             dict(
#                 candidates=search_channel_list,
#                 the_maximum_channel=the_maximum_channel))
...
#             return min(
#                 self.search_btn_ratio_list, key=lambda x: abs(x - btn_ratio))
#         else:
#             return btn_ratio
...
#                 raise NameError(
#                     'structure_info must have nbitsA and nbitsW\n%s' %
#                     (structure_info))
...
#         random_mutate_method = self.minor_method_mutator(
#         ) if minor_mutation else self.method_mutator()
...
#                     'structure_info must have nbitsA and nbitsW\n%s' %
#                     (structure_info))
#             new_nbitsA_list = self.nbits_list_mutator(structure_info['nbitsA'],
#                                                       structure_info['L'])
#             new_nbitsW_list = self.nbits_list_mutator(structure_info['nbitsW'],
#                                                       structure_info['L'])
...
#                 new_L = min(
#                     int(self.budget_layers // 3 //
#                         (len(structure_info_list) - 3)), new_L)

#             if new_L != old_L:
#                 structure_info = revise_nbits_for_layers(
#                     old_L, new_L, structure_info)
