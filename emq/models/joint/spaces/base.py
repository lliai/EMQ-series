...
from .space_utils import (__check_block_structure_info_list_valid__,
                          adjust_structures_inplace)
    ...
    def mutate(self,
        ...
        for mutate_count in range(self.block_num):
            ...
            for idx in range(len(block_structure_info_list)):
                ...
                mutated_block = self.mutators[
                    block_structure_info_list[random_id]['class']](
                        random_id,
                        block_structure_info_list,
                ...
                new_block_structure_info_list = copy.deepcopy(
                    block_structure_info_list)
                new_block_structure_info_list[random_id] = mutated_block

                adjust_structures_inplace(new_block_structure_info_list,
                                          self.image_size)
                if __check_block_structure_info_list_valid__(
                        new_block_structure_info_list, self.budget_layers):
                    break
