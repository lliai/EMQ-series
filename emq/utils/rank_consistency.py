...
def minmax_n_at_k(true_scores, predict_scores, ks=[0.01, 0.05, 0.10, 0.50]):
    """
    1. The input of this function is a list of true_scores and a list of predict_scores.
    2. We use np.argsort to get the rank of the true_scores in descending order.
    3. We use the rank of the true_scores to get the rank of the predict_scores.
    4. The last two lines are to calculate the minn and maxn for each k.
    """
...
def spearman_top_k(true_scores, predict_scores, ratios=[0.2, 0.5, 1]):
    ...
    for ratio in ratios:
        ...
        top_inds = predict_inds[:k]
        bottom_inds = predict_inds[num_archs - k:]
        sp_at_topk = stats.spearmanr(predict_scores[top_inds],
                                     true_scores[top_inds]).correlation
        sp_at_bottomk = stats.spearmanr(predict_scores[bottom_inds],
                                        true_scores[bottom_inds]).correlation
        # [ratio, k, P@topK, P@bottomK, KT in predicted topK, KT in predicted bottomK]
        patks.append((ratio, k, sp_at_topk, sp_at_bottomk))
        running_spearman.append(sp_at_topk)
    return sum(running_spearman) / len(running_spearman), running_spearman
