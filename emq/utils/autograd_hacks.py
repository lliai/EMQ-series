...
def _capture_activations(layer: nn.Module, input: List[torch.Tensor],
    ...
    assert _layer_type(
        layer
    ) in _supported_layers, "Hook installed on unsupported layer, this shouldn't happen"
...
def compute_grad1(model: nn.Module, loss_type: str = 'mean') -> None:
    ...
    for layer in model.modules():
        ...
        if (layer_type not in _supported_layers) or (not hasattr(
                layer, 'activations')) or (not hasattr(layer,
                                                       'backprops_list')):
            continue

        assert len(
            layer.backprops_list
        ) == 1, 'Multiple backprops detected, make sure to call clear_backprops(model)'
...
def compute_hess(model: nn.Module, ) -> None:
    ...
    for layer in model.modules():
        ...
        if layer_type == 'Linear':
            ...
            if layer.bias is not None:
                setattr(layer.bias, 'hess',
                        torch.einsum('oni,onj->ij', B, B) / n)
            ...
            Jb = torch.einsum('onij,onkj->onik', B,
                              A)  # o, n, do, di * Kh * Kw

            Hi = torch.einsum('onij,onkl->nijkl', Jb,
                              Jb)  # n, do, di*Kh*Kw, do, di*Kh*Kw
