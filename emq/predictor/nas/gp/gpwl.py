...
from emq.predictor.nas.gp import BaseGPModel
from emq.predictor.nas.gp.gpwl_utils.convert import *
from emq.predictor.nas.gp.gpwl_utils.vertex_histogram import \
    CustomVertexHistogram
from emq.predictor.nas.gp.gpwl_utils.wl_kernel import WeisfeilerLehman
...
def _compute_pd_inverse(K, jitter=1e-5):
    ...
    while fail_count < max_fail and not is_successful:
        try:
            jitter_diag = jitter * torch.eye(
                n, device=K.device) * 10**fail_count
            K_ = K + jitter_diag
    ...
    if not is_successful:
        print(K)
        raise RuntimeError(
            'Gram matrix not positive definite despite of jitter')
    ...
    def forward(
        ...
        if self.space == 'nasbench301' or self.space == 'darts':
            ...
            X_full = np.concatenate((np.array(self.xtrain_converted), Xnew),
                                    axis=1)
            K_full = torch.tensor(0.5 * torch.tensor(
                self.gkernel.fit_transform(X_full[0]),
                dtype=torch.float32) + 0.5 * torch.tensor(
                    self.gkernel_reduce.fit_transform(X_full[1]),
                    dtype=torch.float32))
            # Kriging equations
        ...
        if self.h == 'auto':
            ...
            if self.space == 'nasbench301' or self.space == 'darts':
                K = (torch.tensor(
                    ...
                    self.gkernel_reduce.fit_transform(xtrain_grakel[1],
                                                      self.y),
                    dtype=torch.float32,
                )) / 2.0
            else:
...
class GPWLPredictor(BaseGPModel):
    ...
    def __init__(
        ...
        super(GPWLPredictor, self).__init__(None, ss_type, kernel_type,
                                            optimize_gp_hyper)
        self.h = h
        self.num_steps = num_steps
    ...
    def _convert_data(self, data: list):
        if self.ss_type == 'nasbench101':
            converted_data = [
                convert_n101_arch_to_graph(arch) for arch in data
            ]
        elif self.ss_type == 'nasbench201':
            converted_data = [
                convert_n201_arch_to_graph(arch) for arch in data
            ]
        elif self.ss_type == 'nasbench301' or self.ss_type == 'darts':
            converted_data = [
                convert_darts_arch_to_graph(arch) for arch in data
            ]
            ...
            raise NotImplementedError('Search space %s is not implemented!' %
                                      self.ss_type)
