...
class CustomVertexHistogram(Kernel):
    ...
    def parse_input(
        ...
        if not isinstance(X, Iterable):
            ...
            if self.sparse_:
                features = csr_matrix((data, (rows, cols)),
                                      shape=(ni, label_length),
                                      copy=False)
            else:
            ...
            if self.oa:
                ...
                for i in range(self.X.shape[0]):
                    for j in range(i, self.X.shape[0]):
                        K[i,
                          j] = np.sum(np.minimum(self.X[i, :], self.X[j, :]))
                        K[j, i] = K[i, j]
            else:
                K = self.X @ self.X.T
        else:
    ...
    def diagonal(self, ):
        ...
        try:
            ...
            if self.sparse_:
                self._X_diag = squeeze(
                    array(self.X.multiply(self.X).sum(axis=1)))
            else:
