...
class WeisfeilerLehman(Kernel):
    ...
    def initialize(self):
        ...
        if not self._initialized['base_graph_kernel']:
            ...
            if base_graph_kernel is None:
                ...
                if not (type(base_graph_kernel) is type
        ...
        if not self._initialized['h']:
            if type(self.h) is not int or self.h < 0:
                raise TypeError("'h' must be a non-negative integer. Got h" +
                                str(self.h))
            self._h = self.h + 1
    ...
    def parse_input(
        ...
        K = []
        for (i,
             g) in enumerate(generate_graphs(label_count, WL_labels_inverse)):
            param = self._params
