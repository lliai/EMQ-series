...
def convert_n101_arch_to_graph(arch, prune_arch=True):
    from emq.predictor.nas.utils.encodings_nb101 import OPS_INCLUSIVE
...
def convert_darts_arch_to_graph(
    ...
    from naslib.search_spaces.darts.conversions import (
        Genotype, convert_naslib_to_genotype)
        ...
        for i in concat:
            if i <= 1:
                G.add_edge(i, n_nodes -
                           1)  # Directly from either input to the output
            else:
                op_number = i - 2
        ...
        for j in range(n_nodes):
            ...
            if G.nodes[j]['op_name'] == 'skip_connect':
                ...
                out_edge = list(G.out_edges(j))[0][
                    1]  # There should be only one out edge really...
