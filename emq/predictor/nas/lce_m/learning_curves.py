...
from emq.predictor.nas.lce_m.curvefunctions import (all_models,
                                                    curve_combination_models,
                                                    model_defaults)
from emq.predictor.nas.lce_m.curvemodels import MLCurveModel
...
class MCMCCurveModelCombination(object):
    ...
    def fit_ml_individual(self, x, y, model_weights):
        ...
        for model in self.ml_curve_models:
            if model.fit(x, y):
                ...
                if not np.isfinite(
                        self._ln_model_prior(model, np.array([params]))[0]):
        ...
        self.ml_params = self._join_theta(all_model_params, sigma,
                                          model_weights)
    ...
    def _ln_model_prior(self, model, params):
        ...
        reshaped_params = [
            params[:, i].reshape((-1, 1)) for i in range(len(params[0]))
        ]
        ...
        if self.monotonicity_constraint:
            ...
            not_monotone = [
                y_mon[i, 0] > y_mon[i, -1] for i in range(len(y_mon))
            ]
            if any(not_monotone):
    ...
    def _ln_likelihood(self, theta, x, y):
        ...
        if self.recency_weighting:
            ...
            ln_likelihood2 = norm.logpdf(
                y - y_model, loc=loc, scale=sigma).sum(axis=1)
    ...
    def normalize_chain_model_weights(self):
        ...
        model_probabilities_chain = (
            model_weights_chain /
            model_weights_chain.sum(axis=2)[:, :, np.newaxis])
        # replace in chain
        self.mcmc_chain[:, :,
    ...
    def _predict_given_params(self, x, model_params, model_weights):
        ...
        if self.normalize_weights:
            ...
            model_ws = (model_weights.transpose() /
                        model_weight_sum).transpose()
        else:
        ...
        for model, model_w, params in zip(self.fit_models,
                                          model_ws.transpose(), model_params):
            params2 = [
                params[:, i].reshape((-1, 1)) for i in range(params.shape[1])
            ]
            params = params2
