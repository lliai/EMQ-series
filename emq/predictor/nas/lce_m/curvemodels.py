...
class CurveModel(object):
    ...
    def __init__(self,
        ...
        self.function_params = [
            param for param in function_args if param != 'x'
        ]
        # set default values:
...
class MLCurveModel(CurveModel):
    ...
    def fit_leastsq(self, x, y, weights, start_from_default):
        try:
            ...
            if status in leastsq_success_statuses:
                ...
                logging.info('leastsq successful for model %s' %
    ...
    def fit_bfgs(self, x, y, weights, start_from_default):
        try:
            ...
            if info['warnflag'] != 0:
                logging.warn('BFGS not converged! (warnflag %d) for model %s' %
            ...
            if any(np.isnan(popt)):
                logging.info(
                    'bfgs NOT successful for model %s, parameter NaN' %
                    self.name)
                return False
            sigma = self.ml_sigma(x, y, popt, weights)
            self.ml_params = np.append(popt, [sigma])
