...
class ConvBNReLU(nn.Module):
    ...
    def get_flops(self, channels, check_range=True, divide=1):
        iC, oC = channels
        if check_range:
            assert (iC <= self.conv.in_channels
                    and oC <= self.conv.out_channels
                    ), '{:} vs {:}  |  {:} vs {:}'.format(
                        iC, self.conv.in_channels, oC, self.conv.out_channels)
        assert (isinstance(self.InShape, tuple)
                and len(self.InShape) == 2), 'invalid in-shape : {:}'.format(
                    self.InShape)
        assert (isinstance(self.OutShape, tuple)
                and len(self.OutShape) == 2), 'invalid out-shape : {:}'.format(
                    self.OutShape)
        # conv_per_position_flops = self.conv.kernel_size[0] * self.conv.kernel_size[1] * iC * oC / self.conv.groups
        conv_per_position_flops = (
            self.conv.kernel_size[0] * self.conv.kernel_size[1] * 1.0 /
            self.conv.groups)
    ...
    def search_forward(self, tuple_inputs):
        assert (isinstance(tuple_inputs, tuple)
                and len(tuple_inputs) == 5), 'invalid type input : {:}'.format(
                    type(tuple_inputs))
...
class ResNetBasicblock(nn.Module):
    ...
    def get_flops(self, channels):
        ...
        if hasattr(self.downsample, 'get_flops'):
            ...
            flop_C = 0
        if (channels[0] != channels[-1] and self.downsample is
                None):  # this short-cut will be added during the infer-train
            flop_C = (
                channels[0] * channels[-1] * self.conv_b.OutShape[0] *
...
class ResNetBottleneck(nn.Module):
    ...
    def get_flops(self, channels):
        ...
        if (channels[0] != channels[-1] and self.downsample is
                None):  # this short-cut will be added during the infer-train
            flop_D = (
                channels[0] * channels[-1] * self.conv_1x4.OutShape[0] *
...
class SearchShapeCifarResNet(nn.Module):
    ...
    def __init__(self, block_name, depth, num_classes):
        ...
        self.message = (
            'SearchShapeCifarResNet : Depth : {:} , Layers for each block : {:}'
            .format(depth, layer_blocks))
        ...
        for stage in range(3):
            ...
            assert (cur_block_choices[-1] == layer_blocks
                    ), 'stage={:}, {:} vs {:}'.format(stage, cur_block_choices,
                                                      layer_blocks)
            self.message += (
                '\nstage={:} ::: depth-block-choices={:} for {:} blocks.'.
                format(stage, cur_block_choices, layer_blocks))
        ...
        self.register_parameter(
            'depth_attentions',
            nn.Parameter(
                torch.Tensor(3, get_depth_choices(layer_blocks, True))),
        )
    ...
    def get_flop(self, mode, config_dict, extra_info):
        ...
        if mode == 'genotype':
            with torch.no_grad():
                depth_probs = nn.functional.softmax(
                    self.depth_attentions, dim=1)
    ...
    def get_arch_info(self):
        string = (
            'for depth and width, there are {:} + {:} attention probabilities.'
            .format(len(self.depth_attentions), len(self.width_attentions)))
