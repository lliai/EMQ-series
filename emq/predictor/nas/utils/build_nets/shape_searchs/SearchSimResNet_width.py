...
class SimBlock(nn.Module):
    ...
    def get_flops(self, channels):
        ...
        if (channels[0] != channels[-1] and self.downsample is
            ...
            flop_C = (
                channels[0] * channels[-1] * self.conv.OutShape[0] *
    ...
    def search_forward(self, tuple_inputs):
        ...
        assert (indexes.size(0) == 1 and probs.size(0) == 1
                and probability.size(0)
                == 1), 'invalid size : {:}, {:}, {:}'.format(
...
class SearchWidthSimResNet(nn.Module):
    ...
    def __init__(self, depth, num_classes):
        ...
        self.message = (
            'SearchWidthSimResNet : Depth : {:} , Layers for each block : {:}'.
