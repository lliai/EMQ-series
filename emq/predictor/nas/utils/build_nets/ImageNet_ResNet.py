...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.message = (
            ...
            format(block, layers, deep_stem, num_classes))
    ...
    def _make_layer(self, block, planes, blocks, stride, groups, base_width):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            if stride == 2:
                ...
                raise ValueError(
                    'invalid stride [{:}] for downsample'.format(stride))

        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample, groups,
                  base_width))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, 1, None, groups, base_width))
