...
class NASNetworkSETN(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(
                3, C * stem_multiplier, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(C * stem_multiplier),
        )

        # config for each layer
        layer_channels = ([C] * N + [C * 2] + [C * 2] * (N - 1) + [C * 4] +
                          [C * 4] * (N - 1))
        layer_reductions = ([False] * N + [True] + [False] * (N - 1) + [True] +
                            [False] * (N - 1))
    ...
    def forward(self, inputs):
        normal_hardwts = nn.functional.softmax(
            self.arch_normal_parameters, dim=-1)
        reduce_hardwts = nn.functional.softmax(
            self.arch_reduce_parameters, dim=-1)
