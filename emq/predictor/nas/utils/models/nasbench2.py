...
def gen_searchcell_mask_from_arch_str(arch_str):
    ...
    nodes = [[op_and_input.split('~') for op_and_input in node]
             for node in nodes]
    ...
    for curr_node_idx in range(len(nodes)):
        for prev_node_idx in range(curr_node_idx + 1):
            _op = [
                edge[0] for edge in nodes[curr_node_idx]
...
def get_model_from_arch_str(arch_str,
                            num_classes,
                            use_bn=True,
                            init_channels=16):
...
def get_super_model(num_classes, use_bn=True):
    net = NAS201Model(
        arch_str=arch_str, num_classes=num_classes, use_bn=use_bn)
    return net
...
class NAS201Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.reduction1 = reduction(
            in_channels=stem_ch, out_channels=stem_ch * 2)
        ...
        self.reduction2 = reduction(
            in_channels=stem_ch * 2, out_channels=stem_ch * 4)
        ...
        self.top = top(
            in_dims=stem_ch * 4, num_classes=num_classes, use_bn=use_bn)
    ...
    def get_prunable_copy(self, bn=False):
        model_new = get_model_from_arch_str(
            self.arch_str, self.num_classes, use_bn=bn)
