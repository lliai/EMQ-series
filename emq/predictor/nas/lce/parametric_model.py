...
def optimize_model_class(fn_class, deg_freedom, x, y, bounds=None):
    ...
    def likelihood(parameters):
        return (len(x) / 2 * np.log(2 * np.pi) +
                len(x) / 2 * np.log(parameters[-1]**2) + 1 /
                (2 * parameters[-1]**2) * sum(
                    (y - fn_class(parameters)(x))**2))

    opt_model = minimize(
        likelihood,
        np.array([1] * (deg_freedom + 1)),
        method='L-BFGS-B',
        bounds=bounds)
    return opt_model['x']
...
model_name_list = ['logloglinear', 'logpower', 'mmf', 'hill3']
...
model_config['mmf'] = {'model': mmf, 'deg_freedom': 4, 'bounds': None}

model_config['exp4'] = {'model': exp4, 'deg_freedom': 4, 'bounds': None}
