...
from emq.predictor.nas.lce.parametric_model import ParametricModel
...
class ParametricEnsemble:
    ...
    def perturb_params(self, params, weights, sigma_sq, var):
        ...
        for model in self.parametric_models:
            perturbed_params[model.name] += np.concatenate([
                perturbation[pos:pos + model.degrees_freedom],
                np.zeros((1, ))
            ])
    ...
    def mcmc(self, x, N=10000, var=0.0001, fit_weights=False, verbose=False):
        ...
        self.fit(x,
                 fit_weights)  # initialize with mle estimates for each model
        ...
        for t in range(N):
            ...
            for j in range(curvelen):
                jth_error = self.predict(
                    j + 1, params=params, weights=weights) - x[j]
            ...
            for j in range(curvelen):
                jth_error = (
                    self.predict(
                        ...
                        weights=candidate_weights) - x[j])
                point_likelihood = norm.pdf(
                    jth_error, scale=np.sqrt(candidate_sigma_sq))
                min_point_likelihood = min(min_point_likelihood,
                                           point_likelihood)
            ...
            if self.predict(
                    curvelen + 1,
                    params=candidate_params,
                    weights=candidate_weights) > self.predict(
                        1, params=candidate_params, weights=candidate_weights):
