...
def find_measures(
    ...
    **kwargs,
    ...
    def sum_arr(arr):
        sum = 0.0
        assert len(arr) > 0
        if isinstance(arr[0], Tensor):
            for i in range(len(arr)):
                sum += torch.sum(arr[i])
            return sum.item()
        elif isinstance(arr[0], (np.float64, np.float32, np.float16)):
            for i in range(len(arr)):
                sum += arr[i]
            return sum
    ...
    for k, v in measures_arr.items():
        if k in [
                ...
                'linear_region', 'condnum', 'ntk_trace', 'nst', 'qe_score',
                'jacobian_trace', 'logits_entropy', 'hessian_trace', 'orm',
                'bit_sum', 'bops', 'bparams'
        ]:
            measure_score = v
        else:
